...
_base_class = virtualbox.library.IMouse if virtualbox else object


class VirtualBoxController(_base_class):
    def __init__(self):
        super().__init__()
        self.vm = None
        self.session = None
        ...
        if virtualbox is None:
            self.logger.warning("virtualbox module not available")
            return
        try:
